...
fn fill_code_shape<Vm: VirtualMachine + VirtualMachineMustHaveExec<Vm> + VirtualMachineMustHaveName<Vm>>(
    vm: &mut Vm,
    shape: CodeShape,
) -> Code<Vm> {
    match shape {
...
/// Returns one random atom
fn fill_code_shape_no_name<Vm: VirtualMachine + VirtualMachineMustHaveExec<Vm>>(
    vm: &mut Vm,
    shape: CodeShape,
) -> Code<Vm> {
    match shape {
