...
mod tests {
    ...
    context_tests! {
        ...
        test_code_mutate: ("( CODE.QUOTE ( A ( ( B ) C ) D ) CODE.MUTATE )", "( CODE.QUOTE ( ( INTEGER.LESS EXEC.POP ) CODE.NTHCDR BOOL.SWAP EXEC.IF ) )", vec![]),
        test_code_mutate_no_name: ("( CODE.QUOTE ( 1 ( ( 2 ) 3 ) 4 ) CODE.MUTATENONAME )", "( CODE.QUOTE ( ( CODE.DONCOUNT INTEGER.LESS ) FLOAT.MIN EXEC.POP INTEGER.PRODUCT ) )", vec![]),
        ...
        test_code_rand_points: ("( 5 CODE.RAND )", "( CODE.QUOTE ( FLOAT.EQUAL INTEGER.LESS EXEC.POP ) )", vec![]),
        test_code_rand_no_name_points: ("( 5 CODE.RANDNONAME )", "( CODE.QUOTE ( FLOAT.YANKDUP FLOAT.EQUAL CODE.DONCOUNT ) )", vec![]),
