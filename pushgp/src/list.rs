...
mod tests {
    ...
    fn not_parsable() {
        let vm = new_base_vm();
        let result = vm.parse("( DOESNT WORK");
    ...
    fn code_display() {
        ...
        let vm = new_base_vm();
        let (_, code) = vm.parse("( ( TRUE 0.012345 -12784 a_name ) BOOL.AND )").unwrap();
    ...
    fn extract_point() {
        ...
        assert_eq!(code.extract_point(1), Extraction::Extracted(vm.must_parse("A")));
        assert_eq!(code.extract_point(2), Extraction::Extracted(vm.must_parse("( B )")));
        assert_eq!(code.extract_point(3), Extraction::Extracted(vm.must_parse("B")));
    }
    ...
    fn code_discrepancy_items() {
        ...
        let code = vm.must_parse("( ANAME ( 3 ( 1 ) ) 1 ( 1 ) )");
        let items = code.discrepancy_items();
        assert_eq!(1, *items.get(&vm.must_parse("ANAME")).unwrap());
        assert_eq!(1, *items.get(&vm.must_parse("( 3 ( 1 ) )")).unwrap());
        assert_eq!(1, *items.get(&vm.must_parse("3")).unwrap());
        assert_eq!(2, *items.get(&vm.must_parse("( 1 )")).unwrap());
        assert_eq!(3, *items.get(&vm.must_parse("1")).unwrap());
    ...
    fn code_len() {
        ...
        assert_eq!(0, vm.must_parse("( )").len());
        assert_eq!(1, vm.must_parse("( A )").len());
        assert_eq!(2, vm.must_parse("( A B )").len());
        assert_eq!(2, vm.must_parse("( A ( B C ) )").len());
    ...
    fn replace() {
        ...
        assert_eq!(
            &vm.must_parse("B"),
            &vm.must_parse("A").replace(vm.must_parse("A").as_ref(), vm.must_parse("B").as_ref())
        );
        assert_eq!(
            &vm.must_parse("( B )"),
            &vm.must_parse("( A )").replace(vm.must_parse("A").as_ref(), vm.must_parse("B").as_ref())
        ...
        assert_eq!(
            &vm.must_parse("( B B )"),
            &vm.must_parse("( A A )").replace(vm.must_parse("A").as_ref(), vm.must_parse("B").as_ref())
        );
        assert_eq!(
            &vm.must_parse("B"),
            &vm.must_parse("( A )").replace(vm.must_parse("( A )").as_ref(), vm.must_parse("B").as_ref())
        ...
        assert_eq!(
            &vm.must_parse("( A A ( A A ) )"),
            &vm.must_parse("( A ( B ) ( A ( B ) ) )")
                .replace(vm.must_parse("( B )").as_ref(), vm.must_parse("A").as_ref())
        );
    }
}
