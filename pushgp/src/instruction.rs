...
pub trait InstructionTrait<C> {
    ...
    // updates to the Context. Returns the number of 'points' required to run the instruction which is a measure of the
    // function's cost. A program can only execute a certain number of points before terminating
    fn execute(context: &mut C);

    fn add_to_table(table: &mut InstructionTable<C>) {
...
// pub fn parse_code_instruction<L: crate::Literal<L>>(input: &str) -> IResult<&str, Code<L>> {
//     let (input, inst) = Instruction::nom_tag(input)?;
//     Ok((input, Code::Instruction(format!("{}", inst))))
// }

// #[cfg(test)]
// mod tests {
//     use crate::{Configuration, ConfigureAllInstructions};
